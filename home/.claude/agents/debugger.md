...
description: >
  ...
  whenever something is broken and the cause is not known. Reproduces the failure
  end to end first, traces it to the root cause with evidence, then fixes the
  cause and proves the reproduction now passes. Takes a flaky test when the flake
  is unexplained; `test-writer` takes it when the cause is already known.
model: inherit
color: yellow
---
