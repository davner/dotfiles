...
- `homebrew.onActivation.upgrade = true` alongside it is also intentional, and the pair is easy to get wrong. `autoUpdate` only refreshes Homebrew's metadata; without `upgrade`, an already-installed cask is left at whatever version it landed on and a rebuild never moves it. That is how the `claude-code` cask sat at an old version while Claude Code nagged about a newer one on every launch. Do not drop it because rebuilds got slower. It still cannot outrun the cask itself, which publishes behind upstream: an installed version below the latest npm release is the cask lagging, not this config failing, so compare `brew info --cask claude-code` against `npm view @anthropic-ai/claude-code version` before chasing it. `~/.claude.json` may also report `installMethod: native` left over from an older hand-install under `~/.local/share/claude/versions`; PATH resolves to the cask binary, so those builds are dead weight rather than what runs.
...
impeccable check` says whether you are behind. Both `impeccable` subcommands
resolve against the working directory, so run them from `~`; anywhere else they
report `Impeccable is not installed in this project` and exit without checking.

Never add `-a` to an `npx skills add` line. Naming a single agent switches the
CLI to copy mode: the skill lands as a real directory under `~/.claude/skills/`
with no `~/.agents/` entry, which is indistinguishable from how `impeccable`
installs and leaves every other harness unable to see it, since the rest read
`~/.agents/skills/` directly. `npx skills update -g` still sees a skill
installed that way, because it tracks installs in `~/.agents/.skill-lock.json`
rather than by location, so the layout is the only symptom. The fix is `npx
skills remove <name> -g -y` followed by the install line above.
